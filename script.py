...
# 세종은 발주기관에 묶인 증거로만 가린다 — 기관 토큰의 지역 링크, 기관 토큰 바로 뒤의 지역, 평문의 세종 부서.
...
# 시·도 기관 토큰 바로 뒤의 지역 토큰. `접수장소: [수요기관(지방정부)] [지역:r1|…]` 처럼 기관의 소재지다.
AGENCY_SEAT = re.compile(r"\[수요기관\((?:" + "|".join(PROVINCE_AGENCY)
                         + r")\)(?:\|지역=r\d+)?\]\s*\[지역:r\d+\|[^\]]*?광역=([^\]|]+)\]")
...
def _is_sejong(text, first):
    ...
    묶인 증거는 셋이다 — 발주기관 토큰의 `|지역=rN` 링크, 시·도 기관 토큰 바로 뒤의 지역 토큰
    (기관의 소재지, 무라벨 `PPS-D-017264`), 평문의 세종 부서(`세종특별자치시 회계과`, `PPS-D-016374`).
    기관에 묶이지 않은 지역 토큰·참가 가능 지역 목록(`PPS-D-007109`)·제한 지역은 현장·납품지이지
    발주처가 아니다(지방 시행규칙 제25조제3항). 그래서 현장 토큰이 세종 부서 문구를 가리지 않는다(`PPS-D-016496`).
    """
    regions = dict(WIDE_OF.findall(text))
    bound = set(AGENCY_SEAT.findall(text))
    if first.group(2) in regions:
        bound.add(regions[first.group(2)])
    return SEJONG in bound or bool(SEJONG_OFFICE.search(text))
