...
# 공고문에 지역 토큰이 하나도 없으면 평문이 세종 부서(`세종특별자치시 회계과`)를 발주처로 적었는지로 본다.
...
def _is_sejong(text, first):
    ...
    # 세종 발주 공고는 지역 토큰이 하나도 없기도 하다(무라벨 `PPS-D-016374`). 그때는 평문이 세종의
    # 부서를 발주처로 적었을 때만 세종이다(`세종특별자치시 회계과`). 참가 가능 지역 목록의 세종
    # (`PPS-D-007109`)도, 제한 지역이 세종 하나인 것도 발주처의 증거가 아니다 — 지역제한은
    # 현장·납품지의 시·도로 건다(지방 시행규칙 제25조제3항).
