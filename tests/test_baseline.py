        ...
        for positives in (set(), {1, 3}):
            ...
            class Selective(baseline.MockRunner):
                def chat(self, batch, items=None):
                    ...
                    if items is not None:   # N2 금액 구간 호출은 facts 없이 두 칸만 낸다
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                # 합동 1회 → v13 추가 호출(선택된 공고가 있을 때만) → N2 금액 구간 호출 셋.
                # 한 공고는 정확히 한 구간에만 들어가므로 구간별 건수의 합이 입력 건수다.
                head = 1 + bool(positives)
                self.assertEqual(calls[:head], [(None, 4)] + [(["v13"], 2)] * bool(positives))
                # 남은 호출은 전부 N2 금액 구간 호출이다. 빈 구간은 호출되지 않으므로
                # 순서만 유지되고, 한 공고는 정확히 한 구간에 들어가 합계가 입력 건수다.
                bands = [c[0][0] for c in calls[head:]]
                ...
                self.assertEqual(sum(c[1] for c in calls[head:]), 4)
