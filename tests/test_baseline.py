...
class BaselineTests(unittest.TestCase):
    ...
    def test_scope_gates_lower_only_the_out_of_scope_positives(self):
        ...
        # **기준액 이름이 붙어도 실적 구절 밖의 비율은 세지 않는다.** 한 인용에 실적 금액과
        # 다른 의무의 비율이 같이 있으면 보증금 비율을 실적 배수로 읽는다(리뷰 P1).
        self.assertEqual(judged("v3", "입찰참가자격: 유사용역 실적 3억원 이상인 업체, "
                                      "입찰보증금은 사업비의 5% 이상 납부",
                                meta={"입찰추정가격": 100_000_000}), 1)
        # 같은 기준액 비율이라도 실적 구절 안이면 요구 배수다.
        self.assertEqual(judged("v3", "유사용역 실적은 사업비의 50% 이상",
                                meta={"입찰추정가격": 100_000_000}), 0)
        # 연결을 확정할 수 없으면 비율로 내리지 않는다 — 인용에 실적이 아예 없는 경우다.
        self.assertEqual(judged("v3", "가. 입찰 금액의 100% 이상",
                                meta={"입찰추정가격": 147_905_232}), 1)
