...
두 회차 결과는 다음 게이트이며, 최종 채택은 착수서 §6 의 무라벨 발화율·시간·독립 리뷰까지 본다.
후보는 프롬프트만 바꾸며 채택 결정이 아니다. [보고서](README.md)가 준비 근거를 소유한다.
이 문서는 실행 조작을 소유한다. 두 회차가 무엇을 가르는지와 결과 감사·판정 순서는
[두 회차 실행 설계](../../../docs/tasks/a8-v20-two-episode-run.md)가 소유한다.
...
4. 실행 셀(인덱스 13): 실행기가 생성 호출 전에 예산을 재고 `budget.json`을 남긴다.
   ...
   통과하면 control→a8 각 dev 200건. 군별 단계 상한 339.178초/200건이다.
   회차가 `diagnostics.jsonl`에 루트·군·청크·실제 호출·파싱을 순서대로 남기고,
...
6. 회차 1 완료·시간 통과 후 런타임을 삭제하고 새 A100 런타임에서 같은 노트북,
...
F1은 company 응답 + 보관 기본/SME 응답의 혼합 CPU 재생이다. 전체 GPU·서버 점수가 아니다.
