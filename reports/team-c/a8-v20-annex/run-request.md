...
독립 리뷰는 라운드 3까지 `머지 불가`다. v13 근거 수리는 **검증된 인용 요구로 결정됐으나 미구현**이며
별도 PR로 나온다. **이 회차를 돌리는 것은 막지 않는다** — 원응답을 보존하면 수리 후 재생으로
다시 채점할 수 있다. 다만 수리와 재확인 전의 결과는 채택 근거로 쓰지 않는다.
