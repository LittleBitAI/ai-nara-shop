...
- 실행 코드 **`5422ae514506ae7a08950d30ed2a437225ee1870`** (노트북 `REPO_REF`).
  출력 예약 1,024·공통 예산 15,296·v13 근거 수리·리뷰 7라운드 반영에 더해
  **실제 모델 호출 관측과 회차 뒤 CPU 감사**, 그리고 **PR #87 라운드 1~7 수리**
  (수출 경계를 출구 하나로 모은 것·생산자 집합 대조·본문 결속·응답 짝·전송 고정·
  `schema_sha256`·`payload_sha256`·`gain_kind`·루트 mode)를 포함한다.
  **더 오래된 커밋으로 돌리면 감사의 `gain_kind` 가 기전을 못 세거나 관측이 없다.**
- [고정 Colab 노트북](https://colab.research.google.com/github/LittleBitAI/ai-nara-shop/blob/489f19fb2dd896f846b2f92f30bfffede79777a3/notebooks/colab-a8-v20-annex.ipynb),
  커밋 `489f19fb2dd896f846b2f92f30bfffede79777a3`, sha256 `e4cfba43…`.
...
기존 패키지·모델·샘플링·문서 예산을 그대로 쓴다. **출력 예약만 두 군 모두 1,024로 내려
프롬프트 예산이 15,296이다**(운영 2,048/14,272와 다르다). 설치·다운로드·적재 시간은 단계 시간과 별개다.
