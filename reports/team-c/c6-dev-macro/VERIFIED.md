...
(c) 회차 간 churn 은 이 표에 없다. 아래에서 그 이유를 적는다.

판정은 §3-1 의 표가 소유한다. 금지선(E·D1)과 발동 기준(Z1·Z2·Z3)은 전부 통과했고,
C·D 의 미달은 통과선이 다른 출력에서 나온 탓이며 기각 사유가 아니다 — 그 근거도 §3-1 에 있다.

### 2-1. (c) 회차 간 churn 은 이 후보에 해당하지 않는다

이 후보는 후처리만 바꾸는 재생 후보다. `docs/workflow.md` W5 가 그런 후보에 두 번째
회차를 요구하지 않는다 — "재생 후보는 같은 원응답의 기준선·후보 비교라 churn이 없으므로
두 번째 회차가 없습니다." `reports/runs/reproducibility.md` 의
「재생 비교에는 이 값을 쓰지 않습니다」 절도 같은 것을 못박는다 — 같은 원응답을 재생한
두 CSV 의 차이에는 churn 이 없고, 그 범위는 서로 다른 모델 실행을 견줄 때만 쓴다.

§3 의 측정은 전부 같은 원응답 위의 재생 비교다. 그래서 (c) 를 답한 것으로 적지 않고,
통과 조건에서도 뺐다(§3-1 에 D2 행이 없다).

### 2-2. 참고 관측 — 그 회차의 두 패스는 12셀 갈렸다

판정에 쓰지 않는 별도 관측이다. 회차 `colab-1790318892216968298` 의 `dev` 와
`dev-debug` 의 `run_report.json` 을 대조하면 코드·입력·레코드·시드·온도가 전부 같고
`debug_responses` 하나만 다르다.
...
그 두 패스는 12셀이 갈렸다(Macro 0.7742 vs 0.7398). 같은 코드·같은 입력인데도 모델
출력이 흔들린다는 관측이고, `d73cfa1` 의 등록문이 적은 값이다.

이것을 (c) 의 답이나 D2 의 판정으로 쓰지 않는 이유 둘.

- 이 후보는 재생 후보라 churn 이 판정에 들어가지 않는다(§2-1).
- 두 패스는 같은 회차 안의 두 통과이고, `reproducibility.md` 의 17~45셀 범위는 서로
  다른 회차를 견준 값이다. 같은 자리에 놓으려면 그 범위 자체를 다시 세워야 하는데,
  그 문서가 그 값을 소유한다.

기록해 두는 이유는 하나다 — 한 쌍의 Macro 차이를 신호로 읽으면 안 된다는 것을
이 회차가 다시 보여 준다. 0.034 는 같은 코드에서 나온 차이다.
...
- `dev` 와 `dev-debug` 를 같은 저울로 읽지 않는다. §3~§4 의 수는 전부 `dev-debug`
  재생이고, `dev` 회차가 낸 0.7742 와 나란히 놓지 않는다.
- 회차 간 churn. 이 후보는 재생 후보라 그것을 묻지 않는다(§2-1). §2-2 의 12셀은
  판정에 안 쓰는 참고 관측이다.
