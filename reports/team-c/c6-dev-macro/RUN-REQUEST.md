# C6 — 통합 후보 GPU 회차 실행 요청서 (닫혔다 — 회차를 안 돌린다)
...
> ## 이 요청서는 실행하지 않는다
>
> §0 의 네 물음이 다른 회차로 이미 답해졌다. PR #142 가 이 세 규칙을 운영
> `script.py` 에 포팅했고(`ca352d6` — "port #139's v18/v16/v11 rules"), 그 코드로
> GPU 회차 `colab-1790318892216968298` 가 돌았다. 그 회차의 새 모델 출력 위에서
> 이 후보만 분리해 재니 아래와 같다. 자세한 것은 `VERIFIED.md` 가 소유한다.
>
> | | 새 출력 위 |
> | --- | --- |
> | 기준 `c68eb00` → 회차코드 `072cd28` | 0.725008120964 → 0.732858256939 (+0.007850135975) |
> | 바뀐 셀 | 6 · C 항목 밖 0 · TP 줄어든 항목 0개 |
>
> 회차 브랜치 `run/c6-dev-macro` @ `072cd28` 은 그대로 둔다(지웠다 되살리지 않는다).
> 아래 내용은 무엇을 어떻게 판정하려 했는지의 기록이며, 그 기준으로 실제 판정한
> 결과가 `VERIFIED.md` 다. 새로 회차를 돌릴 일이 생기면 이 문서를 그대로 쓰면 된다.
...
| (d) | 대상 밖 항목의 회귀가 새 출력에서도 0 인가 | 재생은 한 벌의 출력만 본다. 기준↔후보는 0 이어야 한다(D1) |

회차 간 churn 은 묻지 않는다. 이 후보는 후처리만 바꾸는 재생 후보라
`docs/workflow.md` W5 가 두 번째 회차를 요구하지 않는다 — "재생 후보는 같은 원응답의
기준선·후보 비교라 churn이 없으므로 두 번째 회차가 없습니다." 같은 원응답 위에서
기준↔후보는 결정적이고, 그 차이가 곧 후보의 효과다
(`reports/runs/reproducibility.md` 의 「재생 비교에는 이 값을 쓰지 않습니다」).
...
기준 커밋의 작업 트리 판을 쓰면 안 된다. 기준 커밋 `c68eb00` 에도
`c6-integrated.diff` 가 있지만 그것은 PR #139 가 실은 옛 판(`9038380` 기준)이고,
그 커밋의 `script.py` 에 안 붙는다. 실제로 돌려 확인했다.

```
patching file script.py
Hunk #1 FAILED at 715.
1 out of 1 hunk FAILED -- saving rejects to file script.py.rej
```

B 의 번들이 같은 함수에 `v11_absence_observed()` 를 넣어 문맥이 어긋난다. 이 폴더의
diff 는 `c68eb00` 위에서 다시 만든 것이고 작업 브랜치에만 있다.

그래서 패치를 작업 트리가 아니라 `git show` 로 읽는다. `git show <ref>:<path>` 는
지금 무엇이 체크아웃돼 있든 그 ref 에서 파일을 꺼내므로 detach 상태에서도 돈다.

> 이 자리는 이유가 한 번 바뀌었다. 기준이 `9038380` 이던 때는 그 커밋에 diff 가
> 없어서 작업 트리 경로를 못 썼고, 지금은 있는데 낡아서 못 쓴다.
> 답은 같다 — 작업 브랜치에서 `git show` 로 읽는다.

```bash
git switch --detach c68eb008cad9378bc4e71a763a1c576f8c4d6950
git show feat/c-dev-macro-rebase:reports/team-c/c6-dev-macro/c6-integrated.diff | patch -p1 --binary
...
PR 이 머지된 뒤라면 작업 브랜치 대신 `origin/main` 을 써도 된다. 둘 중 그 파일이
...
한 번만 돌린다. 이 후보는 후처리만 바꾸므로 판정은 그 회차의 원응답 위에서
기준 코드와 후보 코드를 각각 재생해 견주는 것으로 끝난다 — 같은 모델 출력 위에서는
코드 차이가 결정적이라 흔들림이 섞이지 않는다.

회차 ZIP 을 `reports/runs/<run-id>/` 로 등록한다.

기준선은 재생으로 만든다. 회차 1 의 원응답 위에 기준 커밋 `c68eb00` 의 `script.py`
를 박아 재생한 것이 그 회차의 기준선이다 — 같은 모델 출력 위에서만 코드 효과가 결정적이다.

```bash
git show c68eb00:script.py > <tmp>/script_c68eb00.py
py -X utf8 tools/replay_run.py --case reports/runs/<run1>/dev-debug \
  --script <tmp>/script_c68eb00.py --output-dir <tmp>/run1-base
...
D1 에 churn 범위를 섞지 않는다. 같은 원응답 위 `기준↔후보` 는 코드만 다르므로
결정적이고 대상 밖은 정확히 0 이어야 한다. 섞으면 대상 밖 45셀까지 승인된다.

회차 간 churn 범위(17~45셀, Macro 0.000008~0.010088,
출처 `reports/runs/reproducibility.md`)는 이 표에 없다. 그 범위는 서로 다른 모델 실행을
견줄 때만 쓰는 값이고, 여기 기준은 전부 같은 원응답 위의 재생 비교이기 때문이다.
같은 문서의 「재생 비교에는 이 값을 쓰지 않습니다」 절이 그것을 못박는다.
회차를 두 번 돌려 두 패스를 얻었다면 그 차이는 참고 관측으로 적되 통과 조건으로 쓰지 않는다.
...
| | 기준 (`c68eb00`) | 통합 후보 | 변화량 ← 통과선 |
| --- | --- | --- | --- |
| dev Macro | 0.735764126941 | 0.749407019995 | +0.013642893055 |
| v11 TP/FP/FN | 4/4/2 | 5/6/1 | TP +1 · FP +2 · FN −1 |
| v16 TP/FP/FN | 4/4/2 | 4/2/2 | TP 0 · FP −2 · FN 0 |
| v18 TP/FP/FN | 2/1/5 | 4/2/3 | TP +2 · FP +1 · FN −2 |
| 항목별 TP | — | — | 24항목 중 줄어든 것 0개 ← 금지선 E |
| C 항목 TP (참고) | v10 4 · v11 4 · v12 4 · v13 3 · v14 7 · v15 4 · v16 4 · v17 5 · v18 2 · v20 5 | v11 5 · v18 4 · 나머지 그대로 | v11 +1 · v18 +2 · 나머지 0 |
| C 합계 TP (참고만) | 42 | 45 | +3 — 판정에 쓰지 않는다 |
| 조건 충족 / 변경 기회 / 실제 변경 | — | — | v18 38 / 3 / 3 · v16 2 / 2 / 2 · v11 3 / 3 / 3 |
| D1 | — | — | 0 |
...
판정 코드는 `applicability.py` 의 `BASE_REV` 로 고정돼 있다 — 세 층 모두 "기준 코드가 무엇에
적용될 뻔했나"이므로 회차 코드가 아니라 기준 코드로 센다.

고정 원응답에서 이 스크립트가 내는 값은 다음과 같고, 실제 변경의 합 `3 + 2 + 3 = 8`
