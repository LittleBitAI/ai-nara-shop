...
담당 C · 2026-09-26 · 2026-09-27 갱신 · 한 장이다. 왜 이것을 재는지는 `README.md` 가 소유한다.

## 2차 시도 — 고칠 줄이 없는 전용 노트북을 쓴다

**1차 시도(`colab-1790432295199698396`)는 변동폭 회차가 안 됐다.** 아래 §1·§3 이
요구한 손작업 둘 — `REPO_REF` 박기와 셀 붙여넣기 — 가 안 들어갔고, 노트북 기본
경로만 돌아 `dev`·`dev-debug` 두 통과가 왔다. 그 결과는 버리지 않았다:
`RESULT.md` 가 소유하는 **둘째 관측**이 됐다.

2차부터는 그 손작업을 파일 안에 넣었다. **아래 §1 과 §3 은 이제 안 해도 된다.**

| | |
| --- | --- |
| 링크 | `https://colab.research.google.com/github/LittleBitAI/ai-nara-shop/blob/1928c35b8cfd08bb9731fa3fab02c1f58226290b/notebooks/colab-c-variance.ipynb` |
| 노트북 | `notebooks/colab-c-variance.ipynb` (브랜치 `run/c-variance-n6`) |
| 고칠 줄 | **없다.** 위에서부터 순서대로 다 실행한다 |
| 그 브랜치의 `script.py` | `772ca12` 와 **바이트가 같다**(`git diff --stat 772ca12 1928c35` 가 노트북 한 파일만 든다) |
| 노트북 안의 `REPO_REF` | `772ca1284c8918c1fa4ca5729cc15dfe4dedb2d4` 로 박혀 있다 |
| 예상 소요 | 통과당 **12분**(1차에서 실측 723.9s · 719.8s) × 6 = **약 1시간 12분** + 셋업 |

**이 노트북은 `main` 에 안 올린다**(2026-09-27 결정). N3 의 `b521bb2` 와 같이 커밋
고정 링크를 담는 회차 브랜치로만 둔다 — 상시 제약 「`notebooks/` 는 diff 로만」을
지키고, A 의 머지 판단을 한 건 안 늘린다.

**그래서 `run/c-variance-n6` 브랜치를 지우면 위 링크가 죽는다.** 회차가 끝나고
`RESULT.md` 에 수가 들어갈 때까지 그 브랜치를 정리 대상에서 뺀다. 머지 뒤 정리를
돌리는 사람이 이 줄을 보게 여기 적는다.

기준 노트북에서 뺀 것은 dev·dev-debug 검증 통과(`[14]`~`[18]`)다. 그 둘은
`debug_responses` 가 서로 달라 이 실험의 여섯 통과와 같은 저울이 아니고 24분이
더 든다. 샘플 10건과 `check_live` 는 남겼다 — 40초에 환경이 맞는지 보고 들어간다.

검증 통과를 빼면 `validation.json` 이 없어 `docs/runs.md` 색인의 Macro 가 빈다.
`register_run.py` 는 `submit_sha256` 을 셀 `[4]` 가 무조건 쓰는 `candidate.json`
에서 찾으므로 **등록 자체는 된다.** 이 회차의 점수는 어차피 로컬 채점이 소유한다.

아래 §1 ~ §6 은 기준 노트북으로 손수 돌릴 때의 절차로 남겨 둔다.
