...
### 최종 판정 — **넷**을 **전부** 만족해야 채택 후보다

| # | 무엇 | 판정 |
| --- | --- | --- |
| **A** | **두 회차 모두** 기준 1(v18 FN ≤ 3)과 기준 2(v10~v13 TP 10 유지)를 만족 | 하나라도 한 회차에서 어긋나면 **반려** |
| **A′** | **두 회차 모두** 기준 4 를 만족 — `company_size` 추론 증가분이 **92.193초 안**이고 `company_size_response_count` 가 **200** | 한 회차라도 넘거나 모자라면 **반려** |
...
> 서버 예산은 **나쁜 회차**가 실제로 쓰는 시간으로 정해진다.
...
두 회차가 **같은 수만큼** FN 을 남기되 **다른 공고**에서 남기면 차이가 `0` 이 된다.
...
**예측이 다른 공고 목록**이다. 라벨은 고정이므로 **양성 라벨 공고의 예측이 바뀌면
FN 집합이 바뀐 것**이고, 그 수가 곧 FN 축의 churn 이다.
...
| **`<churn-c>` 의 `items[v18].flipped` 공고 ID 전체** | `c_FN` 의 원자료. **두 회차가 서로 다른 공고를 복구했는지**가 여기서 보인다 |
...
- **`c_FN` 이 0 으로 나올 수도 있다.** 두 회차가 **같은 공고들**에서 FN 을 남기면
  `flipped` 에 양성 공고가 없어 `c_FN = 0` 이고, `Δ > 0` 이면 B 를 통과한다.
  그것은 **그 축이 이 회차 쌍에서 안 흔들렸다**는 뜻이지 잡음이 없다는 증명이 아니다 —
  회차 쌍 하나는 표본 1 이다.
- **`c_FN` 은 v18 만 센다.** v14~v17 의 회차 간 흔들림은 기준 A 가 각 회차에서 따로 보고
  `c_off` 는 대상 **밖**만 센다. **다섯 항목 안의 v18 아닌 흔들림은 어느 기준도 직접
