...
실행은 C(사용자)가 한다. 이 문서의 합격 기준은 회차 전에 고정된 것이며
결과를 보고 고치지 않는다.
...
기존 `clause_quotes`·`qualification_role` 과 같은 방식으로 고쳤다.
...
`tests/test_c_competitive_row_diff.py` 10건이 이것을 고정한다 — 바이트 동일,
새 경로에서 필수, legacy·플래그 꺼짐 경로에서 제외, 검증 실패 시 `unknown`(`general` 아님),
그리고 diff 가 저장소에 적용돼 있지 않음까지 본다.
...
`039`·`040`·`041`·`044` 는 `scope` 를 사업명·품명으로 `competitive` 라 했다.
네 공고의 원문에 "경쟁제품" 표기는 0회다.
...
이것은 새 판별축이 아니라 이미 내린 판정에 근거를 요구하는 것이다.
...
재생이 도는 것은 회귀가 없다는 뜻이지 효과를 쟀다는 뜻이 아니다.
...
| --- | --- | --- |
| 미적용 `script.py` | OK · 키 15개 | 버려짐 |
| 적용 `script.py` | OK · 키 16개 | 보존 |
...
| 무엇을 재생하나 | 되는가 | 이유 |
| --- | --- | --- |
| 새 회차를 `--verify` 로 | 된다 | 고정 커밋의 `script.py` 가 쓰이고, `parse_judgment` 의 `company_size_competitive_row` 기본값이 `True` 라 HEAD 재생기가 그 키를 안 넘겨도 새 스키마로 파싱된다 |
| 옛 보관 회차를 diff 적용 상태에서 | 재생기도 그 커밋 것이어야 한다 | HEAD 재생기는 키를 안 넘기고 기본값이 `True` 라 옛 응답이 "사실 필드 결손" 으로 죽는다. 고정 커밋의 `tools/replay_run.py` 가 `settings.get(...)` 로 꺼 준다 |
...
2. 약칭 SHA 를 거부한다. `git rev-parse HEAD` 의 40자리 전체를 쓴다.
...
첫 판은 그 값을 과거 13쌍의 17~45셀로 적었는데, 그것은 다른 코드의 잡음 범위다.
...
넘으면 기준 4 위반이다. 다른 기준을 충족해도 반려한다 — 서버 예산을 넘기면
제출 자체가 불가능하다.
...
1회차의 기준 4 통과는 최종 판정을 대신하지 않는다. 2회차도 같은 기준을 다시 받는다
(`A′`). 1회차에서 봤으니 됐다고 넘기지 않는다 — 시간은 회차마다 다르다.

### 2회차 — 같은 조건이어야 한다

다섯을 1회차와 동일하게 고정한다. 하나라도 다르면 churn 측정이 아니다.
...
>
> 기존 기록이 그것을 보인다. 같은 `code.commit` 으로 두 번 돈 회차 쌍 4쌍이 있고
...
### 최종 판정 — 넷을 전부 만족해야 채택 후보다

| # | 무엇 | 판정 |
| --- | --- | --- |
| A | 두 회차 모두 기준 1(v18 FN ≤ 3)과 기준 2(v10~v13 TP 10 유지)를 만족 | 하나라도 한 회차에서 어긋나면 반려 |
| A′ | 두 회차 모두 기준 4 를 만족 — `company_size` 추론 증가분이 92.193초 안이고 `company_size_response_count` 가 200 | 한 회차라도 넘거나 모자라면 반려 |
| B | v18 FN 감소폭 > v18 FN churn | `≤` 면 반려. 잡음과 구분되지 않는다 |
| C | 대상 밖 바뀐 셀 ≤ 대상 밖 churn | 넘으면 회귀로 반려 |

> `A′` 을 빠뜨렸던 것을 고쳤다. 앞 판의 최종 판정은 기준 1·2 만 두 회차에 걸었고
> 기준 4(시간·응답)를 1회차에서만 봤다. 그러면 2회차가 예산을 넘겨도 채택된다.
>
> 시간은 회차마다 다르다. 같은 코드·같은 입력이라도 GPU 상태와 출력 길이에 따라
> `company_size_inference_seconds` 가 흔들린다. 그래서 양쪽 다 본다.
>
> 판정은 `max` 로 한다 — `max(t₁, t₂) − 247.363 ≤ 92.193` 이고
> `min(cnt₁, cnt₂) = 200` 이어야 한다. 평균을 쓰지 않는다.
> 서버 예산은 나쁜 회차가 실제로 쓰는 시간으로 정해진다.
...
| `c_FN` (v18 FN churn) | 두 회차에서 v18 예측이 다른 공고 중 라벨 v18=1 인 것의 수 | `<churn-c>` 의 `items[v18].flipped` ∩ `dev_labels.csv` 의 v18=1 |
...
python -X utf8 -c "
import csv, json, sys
truth = {r['id']: r for r in csv.DictReader(open(sys.argv[2], encoding='utf-8'))}
item = next(i for i in json.load(open(sys.argv[1], encoding='utf-8'))['items'] if i['item'] == 'v18')
flipped = [i for i in item['flipped'] if truth[i]['v18'] == '1']
print('c_FN =', len(flipped), flipped)
" <churn-c>/comparison.json open/dev_labels.csv
```

판정식은 이것뿐이다.

```text
B :  Δ > c_FN            즉  (6 − max(FN₁, FN₂))  >  c_FN
C :  max(off₁, off₂) ≤ c_off
```

왜 `max` 인가. 두 회차 중 나쁜 쪽으로 판정한다. 좋은 회차를 골라 쓰면
결과를 보고 기준을 고르는 것이다.

왜 같은 축인가. 첫 판은 "v18 FN 감소폭" 을 전체 바뀐 셀 수와 견주게 적었다.
그것은 셀 개수와 FN 개수를 견주는 것이라 판정이 성립하지 않는다.
이제 FN 은 FN 끼리(`Δ` 대 `c_FN`), 대상 밖 셀은 대상 밖 셀끼리(`off` 대 `c_off`) 본다.

### `c_FN` 을 개수 차이로 세면 안 된다 — 실측으로 확인했다

앞 판은 `c_FN = |FN₂ − FN₁|` 이었다. 그것은 공고가 뒤바뀌는 것을 못 본다.
두 회차가 같은 수만큼 FN 을 남기되 다른 공고에서 남기면 차이가 `0` 이 된다.

실제 `compare_runs.py` 로 만들어 확인했다. 기준 CSV 에서 `runA` 는 `038` 만,
`runB` 는 `039` 만 v18=1 로 되살린 두 판을 대조했다.

```text
*v18     2→2       2→2       5→5     0.363636→0.363636    2  PPS-DEV-038, PPS-DEV-039

|FN₂ − FN₁| = |5 − 5| = 0        ← 앞 판의 정의. churn 이 없다고 말한다
flipped ∩ 라벨 v18=1 = 2         ← 실제로는 공고 둘이 통째로 뒤바뀌었다
```

두 회차가 서로 다른 공고를 복구했다는 것은 이 관측이 불안정하다는 뜻이다.
개수만 보면 그것을 놓치고, 안정적이라고 잘못 읽는다. 그래서 집합 차이로 센다.

`flipped` 는 `compare_runs.py` 가 `before[i][index] != after[i][index]` 로 만드는
예측이 다른 공고 목록이다. 라벨은 고정이므로 양성 라벨 공고의 예측이 바뀌면
FN 집합이 바뀐 것이고, 그 수가 곧 FN 축의 churn 이다.

예시(값은 가정이다 — 회차 전에는 모른다).
`FN₁=3 · FN₂=3` 인데 서로 다른 공고면 `Δ = 3`, `c_FN = 2` → `3 > 2` 라 B 통과.
`FN₁=3 · FN₂=5` 이고 뒤바뀐 양성이 4건이면 `Δ = 1`, `c_FN = 4` → B 반려.
...
| v14·v15·v16·v17·v18 다섯 항목의 TP/FP/FN 전부 | 어느 기준도 v18 아닌 항목의 회차 간 흔들림을 직접 재지 않는다. 뒤에 셀 수 있게 남긴다(§9) |
| `FN₁`·`FN₂`·`off₁`·`off₂`·`c_FN`·`c_off` | 기준 B·C 판정. §4-1 의 정의대로 `comparison.json` 에서 읽는다 |
| `<churn-c>` 의 `items[v18].flipped` 공고 ID 전체 | `c_FN` 의 원자료. 두 회차가 서로 다른 공고를 복구했는지가 여기서 보인다 |
| 양 회차의 `company_size_inference_seconds`·`company_size_response_count` | 기준 `A′` 판정. 1회차만 보지 않는다 |
...
| FN 수는 같은데 공고가 다르다 | `FN₁ = FN₂` 인데 `flipped` 에 양성 공고가 있다 | `c_FN` 이 그만큼 커진다. 개수 차이로 세면 0 으로 보인다 — 그래서 집합으로 센다(§4-1) |
| 2회차만 시간이 넘는다 | `t₂ − 247.363 > 92.193` | 기준 `A′` 위반으로 반려. 1회차가 통과했어도 상관없다 |
...
| `register_run.py` 등록 | 미실행. CLI 인자와 두 검사(`len(candidates)!=1`, `source.json` 대조)를 소스로 확인했다 |
...
| 두 회차 반복 | 미실행. §4-1 은 판정 방법을 고정한 것이고 churn 실측값이 아니다 |
...
| B·C 판정식이 계산되는지 | 확인함. `<cmp>/comparison.json` 에 `items[v18].before.fn`·`after.fn`·`changed_cells_off_focus`·`flipped` 가 실제로 있다 |
| `c_FN` 개수 정의의 실패 | 실측으로 재현함. `038` 만·`039` 만 되살린 두 CSV 를 `compare_runs.py` 로 대조하니 `fn 5→5`(개수 차이 0)인데 `flipped` 는 두 공고였다 |
| `c_FN` 계산 명령 | 돌려 봄. 그 대조에서 `c_FN = 2 ['PPS-DEV-038', 'PPS-DEV-039']` 가 나온다 |

TP/FP/FN 전→후가 없다. 후보의 효과를 이 문서가 주장하지 않는다.
`§1` 의 무리별 TP 수는 현재 회차의 실측이고 이 관측의 결과가 아니다.
...
출력에 걸리므로 무라벨 입력에 대한 `company_size` 원응답이 따로 필요하다.
입력 파일은 "제공 목록에 후보 행이 있는 공고의 비율" 같은 입력 측 조건까지만 답한다.

회차가 기준 1~4 를 전부 통과해도 채택은 무라벨 배율을 잰 뒤다.
...
- `unknown` 으로 두는 선택이 v18 FN 을 여는지. 이 관측의 회복 경로는 코드가
  뒤집는 것이 아니라 모델이 스스로 `general` 을 내는 것이다. 코드 쪽은 방어막이다.
...
- `c_FN` 이 0 으로 나올 수도 있다. 두 회차가 같은 공고들에서 FN 을 남기면
  `flipped` 에 양성 공고가 없어 `c_FN = 0` 이고, `Δ > 0` 이면 B 를 통과한다.
  그것은 그 축이 이 회차 쌍에서 안 흔들렸다는 뜻이지 잡음이 없다는 증명이 아니다 —
  회차 쌍 하나는 표본 1 이다.
- `c_FN` 은 v18 만 센다. v14~v17 의 회차 간 흔들림은 기준 A 가 각 회차에서 따로 보고
  `c_off` 는 대상 밖만 센다. 다섯 항목 안의 v18 아닌 흔들림은 어느 기준도 직접
  재지 않는다 — 이 요청서의 판정 목표가 v18 FN 이기 때문이고, 그 대신 §5 가 두 회차의
  대상 항목 TP/FP/FN 을 전부 기록하게 해 뒤에 셀 수 있게 남긴다.
- 두 회차로 충분한지. 회차 쌍 하나로 재는 churn 은 그 자체가 표본 1 이다.
  더 늘리면 정밀해지지만 회차를 늘려 통과시키는 길과 구분이 어려워져 둘로 고정했다.
