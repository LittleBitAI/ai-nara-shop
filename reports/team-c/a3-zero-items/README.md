...
최신: H1은 [두 회차 모두 세 항목 TP=0](run2-facts.md)으로 실패했다.
H2는 [회차 3](run3-facts.md)에서 v18 첫 TP를 얻었으나 v10·v20 TP=0이다.
H3는 [회차 4](run4-facts.md)에서 v10 4/7/3·v18 0/2/7·v20 1/4/4, dev 0.5743735227782543이다.
세 항목 동시 TP>0에는 미달했다. 다음 후보는 [H4 자격 문장 역할](h4-qualification-role.md)이며
새 실제 모델 효과는 미측정이다. 아래는 H1 이력이다.

2026-09-20, glassfish. 첫 live dev 완료·검사 오류 복구, 성능 미채택.
후속 회차 `1789886517580784653`은 dev 200건 추론 뒤 노트북의 옛 v13 보호 조건으로 중단됐다.
공용/A3 검사를 수정했고, 저장 CSV의 CPU 복구 채점은 0.5357800832, v10·v18·v20 TP는 0이다.
[오류 원인·회귀 검사·복구 결과·재개 방법](run-1789886517580784653/README.md).
아래 착수 시점의 미측정 기록은 이 후속 결과와 구분한다.
...
전체 dev 200건에서는 같은 현상이 8건이다. 나머지 4건을 포함한 ID와 원응답은
...
따라서 작업서의 044 법령 인용은 역할을 구별할 원문 표본이고, 이 회차 모델이 실제로
...
추가 호출은 0회다. 기존의 동일 사실 추출을 정확하게 만드는 실험이다.
...
이는 제공 판로지원법 시행령 제2조의2의 기업범주·제한경쟁 조건을 읽기 위한 추출 가설이다.
...
- 무라벨 파일은 이 워크트리에 없어 원본 저장소의 같은 제공 파일을 읽었다.
  ...
  [입력 사전 점검](input-preflight.json)의 `firing_rate_ratio`는 null이다.
...
GPU 런타임·Colab 보안 비밀 `HF_TOKEN`과 모델 읽기 권한을 준비하면 값 수정 없이 실행한다.
...
동일 로컬 환경에서 후보 커밋의 ZIP은 아래 로컬 업로드 후보와 script·submit ZIP SHA-256이 같았다.
후속 실제 Colab ZIP은 해시가 달랐으며 script 해시는 같다. 반복은 실제 회차 ZIP을 기준으로 한다.
전용 노트북은 공용판과 코드 셀의 `REPO_REF`만 다르다. 고정 커밋 clone→번들 생성에서
동일 ZIP을 확인했고, 전용 노트북에 기존 패키징 검사 7개를 적용해 모두 통과했다.
[노트북 검사 기록](notebook-check.json). 모델을 호출한 검증은 아니다.

### 기존 로컬 업로드 방식

로컬 경로: `artifacts/a3-zero-items/0e432eacf041/`.
그 안의 colab-baseline.ipynb를 열고 colab-bundle.zip을 업로드한다.
...
첫 회차는 dev 200건·원응답을 보관한다. 채택 검토 시 동일 ZIP으로 별도 재실행한다.
...
v20은 실제 구매 대상이 소프트웨어 사업인지와 문서의 참가 제한을 구분하는 별도 가설이다.
