...
2026-09-22 갱신: 채택했다. 대조 축은 `script.py` 의 `evidence_refutes()` 안에 있다.
근거 계약을 채택한 뒤 현재 파이프라인에서 다시 재 0.603889319192 → 0.609274806721,
`+0.005385487528` 로 아래 2026-09-21 측정과 같은 값이다. 무라벨 배율 1.206 은 그대로다.

넣을 때 걸린 것 하나 — `evidence_refutes()` 는 맨 위에서 `if not evidence: return False`
로 빠져나간다. v24 오탐의 대부분이 근거 빈 양성이라 그 아래에 두면 대조 검사에 도달하지
못하고 v24 가 `5/28/3` 에 그친다. v24 가지를 그 조기 반환 **위로** 올려야 `4/12/4` 가 된다.

아래 "바뀐 셀 24개" 는 산술 누락이다. 오탐 24건(36→12)에 정탐 1건(5→4)을 더해 **25셀**이고
전부 v24 안이다. 재측정에서 확인했다. 대상 밖 0셀이라는 결론 자체는 맞다.

2026-09-21. 모델을 부르지 않았다. GPU 0초. 회차 `colab-1789902969401579900`(`18f07e5`)의
보관 원응답과 dev 200건으로 잰다. 아래는 채택 전 기록이다.
