...
| --- | --- | --- |
| 6 | 창 안 모든 금액 비교 → 같은 줄 **부가세**를 불일치로 읽음 | — |
| 7 | "첫 금액 하나만" | 산식의 **구성값**을 총액과 비교 (`1,750,000 × 18명 = 31,500,000`) |
...
재생 CSV 가 바이트 동일**이다. 정밀도는 1/3 이다. dev 이득 0, 위험은 비공개에만 있다.
...
다시 켜려면 **필드별 대응**과 **표시 반올림 동등성**이 필요하고, 그 뒤 카나리를 다시 산출해야 한다.
