...
export default function App() {
  ...
  // 화면 둘은 다른 질문에 답한다. 회차 진단은 "무엇을 틀렸나", 법령은 "근거가 어디인가".
  // 한 화면에 쌓으면 법령 칸이 개요 맨 아래에 묻혀 아무도 못 찾는다. 실제로 그랬다.
  ...
  return (
    <div className="app">
      <header className="top">
        ...
        <div className="screens" role="tablist">
          {[['runs', '회차 진단'], ['laws', '법령']].map(([key, label]) => (
            <button key={key} role="tab" aria-selected={screen === key}
                    className={screen === key ? 'on' : ''} onClick={() => setScreen(key)}>
              {label}
            </button>
          ))}
        </div>
      ...
      {screen === 'laws' ? (
        <LawView items={corpus.items} focus={item} onFocus={(next) => open(next, null)} />
      ) : (
        <>
              ...
              {!item ? (
                ...
                <>
                  <div className="panehead">
                    ...
                    <button className="linkish" onClick={() => setScreen('laws')}>
                      법령에서 이 항목의 근거 보기 →
                    </button>
                  </div>
                  {ids.length ? (
