...
- **스키마 수정 성공:** `판정=1` 66건 전부 `violation_found`. 앞 회차 286/370 어긋남이 사라져
  단계 히스토그램을 쓸 수 있다.
- **그런데 재현율이 무너졌다:** v16 6→1, v18 4→2, v20 3→0. 합계 13/18 → **3/18**.
  양성 판정 총수 370 → 66. 같은 모델·같은 공고이고 **바뀐 것은 판정 칸의 모양뿐**이다.
  앞 회차의 "v16 6/6"은 모델 실력이 아니라 느슨하게 물은 결과였다.
...
- 같은 `script.py`의 다섯 번째 측정: dev 0.2202239896. churn 관측을 열두 쌍으로 늘리고
  셀 범위를 25~45로 넓혔다. `compare_runs.py` 상수와 인용 문서 4곳을 함께 고쳤다.
- 결정 기록이 번호 없이 `2026-09-18-run-<run-id>.md`로 나왔다. PR 번호와 안 부딪힌다.
- 미실행: 이 회차는 제출하지 않았다. 세 변수(항목 수·근거 null·출력 형식) 분리는 C3 몫이다.
