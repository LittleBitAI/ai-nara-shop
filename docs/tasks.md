...
**독립 리뷰 라운드 3까지 `머지 불가`** — 라운드 1·2의 P1 다섯은 반영했다. 남은 것은 운영
`script.py:668`의 v13 근거 수리이며(A8 이 만든 것 아님, 현재도 3셀 발화 + 결합 입력에서 신규 24셀)
**검증된 인용 요구로 결정됐으나 미구현**이고 별도 PR로 나온다. 채택 게이트는 그 수리와
재확인 전까지 닫혀 있다. GPU 실행 자체는 막지 않는다. GPU·무라벨·서버 미실행.
