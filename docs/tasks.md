...
PR #87 **라운드 1 에서 P0 하나·P1 넷**을 받아 전부 고쳤다 — 사이드카가 프롬프트·응답
전문을 임의 host 로 조건 없이 보냈고(설계 §3.5 미구현), `schema_sha256` 이 실제 제한
스키마를 증명하지 못했고, 감사가 `payload_sha256` 을 재계산하지 않아 원응답을 바꿔도
통과했고, `gain_kind` 가 인용 상태만 봐 두 가지를 오분류했고, 루트가 `mode=pending` 으로
끝나 실제 회차와 대역 실행이 안 갈렸다. 다섯 가드를 하나씩 되돌려 각각 빨개지는 것을 확인했다.
실행 핀은 코드 `ea9ac8a` · 노트북 `516ddb9`(v13 수리 + 관측·감사 + 라운드 1 수리).
