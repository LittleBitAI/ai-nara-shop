...
그 뒤 **관측·감사 배선**을 얹었다(`feat/a8-observation-capture`) — 회차가 실제 모델 요청을
...
전체 342 통과(기존 환경 실패 3건 동일). **다음 게이트는 A8 두 회차 실행**이고 최종 채택 조건은
착수서 §6 그대로다 — 유망하면 무라벨 6,000건 발화율·시간·독립 리뷰까지 확인한다.
두 회차 성공만으로 채택하지 않는다. GPU·무라벨·서버 미실행.
PR #87 **라운드 1 에서 P0 하나·P1 넷, 라운드 2 에서 P0 셋·P1 하나·P2 둘**을 받아 전부 고쳤다.
...
라운드 4: 응답 결속과 **구조화 필드 위생**이다 — 시작에서 본문만 빼고 응답을 위조하면
...
실제 H4 로그)이고, 검사는 **실제 회차 로그 1,020 이벤트**로 크래시·보존·차단 셋을
...
뒤집었다. 라운드 7: **P1 없이 P0 둘**이고 둘이 같은 원리였다 — 기본을 뒤집고도 평문
예외를 **패턴으로 둔 자리**가 남아 있었다(map 키, `phase`·`error_type`).
**모양이 값을 증명한다고 볼 수 있는 것은 hash 뿐이다** — 나머지는 생산자 집합과 대조한다.
전체 387 통과.
라운드 8: 남은 **모양 예외 네 자리**(자산 키·공고 id·버전·span 이름)를 전부 집합 대조로
바꿨다. 이제 **모양으로 평문을 통과시키는 자리가 0개**다 — 남은 정규식은 hash 와, 조각이
...
`_clean()` 반환값만 비교한 검사가 **소비 경로**를 놓쳤다.
