...
출력: `reports/server-gap-20260925/`의 재현 가능한 CPU 감사와 원인·9/29 전략 보고서.
수정 범위: 해당 보고서 폴더·이 작업 큐·`.wiki/plan-active.md`의 낡은 현재 수치.
통과 조건: 기존 채점기로 같은 원응답의 제출 전후 점수 재계산, 입력 분포 전수 집계,
확정 사실·추론·미측정 분리. 운영 코드 변경·유료 실행·대회 제출 없음.
규칙: 개발/로컬 검증 A1·A2·A4·A6, R3·R5·R8·R9·R11·R15·R22 유지.
결과: 20,200건 집계·두 코드×다섯 재생 완료. 전이 0.322819 재현. 계약방법 재가중으로 큰 격차 설명 불가.
새 라벨·검색 후보·GPU·서버 실행은 제안이며 미실행이다.
...
계약·불변식·완료·시간은 모두 충족했으므로 실행 실패가 아니라 성능 기각이다.
대상 밖 v12 FP 증가·v17 TP 손실·v18 FP 증가가 두 회차에 공통이라 채택 보류 조건도 함께 걸렸다.
회차 보관은 [`a8-v20-1790057577391794027`](../reports/runs/a8-v20-1790057577391794027/)·
[`a8-v20-1790059133832323156`](../reports/runs/a8-v20-1790059133832323156/)이고 화면 항목 여덟 개가 붙었다.
착수서 §6의 남은 게이트(무라벨 6,000건 발화율·시간·독립 리뷰)는 진행하지 않는다.
아래는 회차 전 준비 기록이다.

준비 단계 기록 / 후보·CPU 계약 완료,
...
고정 회차 v20 TP 상한 2 재확인.
독립 리뷰는 라운드 7에서 `머지 허용`을 받았고 PR #84 로 머지됐다(P1 열둘·P2 다섯 반영).
v13 근거 수리도 사용자 결정으로 그 PR에 넣었다 — `script.py:company_size_products()`가
검증된 인용을 요구한다. 실측 Macro −0.002838,
v13 4/9/2 → 3/8/3, 움직인 셀 3개·대상 밖 0셀, 법령 스윕과 결합 경로는 0셀이 됐다.
그 뒤 관측·감사 배선을 얹었다(`feat/a8-observation-capture`) — 회차가 실제 모델 요청을
`diagnostics.jsonl`에 기록하고(capture_protocol 1, 사이드카 투영 1,618 op·생성 키 400),
회차 뒤 같은 런타임의 CPU 감사가 인용 상태와 v20 판정을 갈라 `v20-audit.json`을 남긴다.
추론 불변·모델 재호출 0이고, 먼저 보는 숫자는 `gain_kind` 다 — `fallback_only` 는 기전이 아니다.
전체 342 통과(기존 환경 실패 3건 동일). 다음 게이트는 A8 두 회차 실행이고 최종 채택 조건은
착수서 §6 그대로다 — 유망하면 무라벨 6,000건 발화율·시간·독립 리뷰까지 확인한다.
두 회차 성공만으로 채택하지 않는다. 준비 시점 기준 GPU·무라벨·서버 미실행이었다.
PR #87 라운드 1 에서 P0 하나·P1 넷, 라운드 2 에서 P0 셋·P1 하나·P2 둘을 받아 전부 고쳤다.
라운드 1: 사이드카가 프롬프트·응답 전문을 임의 host 로 조건 없이 보냈고(설계 §3.5 미구현),
`schema_sha256` 이 실제 제한 스키마를 증명하지 못했고, 감사가 `payload_sha256` 을
재계산하지 않아 원응답을 바꿔도 통과했고, `gain_kind` 가 인용 상태만 봐 오분류했고,
루트가 `mode=pending` 으로 끝났다.
라운드 2: 수출 경계를 세 갈래로 우회할 수 있었다 — 유효한 공고 id 에 임의 본문을
붙이면 통과했고(id 는 본문 출처의 근거가 아니다), 로컬 주소가 307 로 외부를 가리키면
검사를 통과한 본문이 다시 나갔고, `run_failed` 의 예외 메시지·traceback 이 metadata
전용 모드로 나갔다(개인 절대경로 포함). 그리고 `gain_kind` 가 v20 의 주 기전인
부재 판정(`preserve → write_1`)을 `other` 로 뒀다 — H4 양성 5건이 그 분기다.
라운드 3: 수출 경계를 또 세 갈래로 우회할 수 있었다 — system·user 사이에 메시지를
끼우면 배열 전체가 나갔고, `trust_env` 때문에 `HTTP_PROXY` 가 로컬 주소까지 가로챘고,
`--session` 기본값이 개인 절대경로였다. 그리고 `_known_systems` 가 set 순서로 prefix 를
골라 진짜 A8 회차가 seed 마다 거부됐고(PYTHONHASHSEED 1~6 중 넷),
`gain_kind` 가 보류의 원인을 열거해 `requirements_complete` 경로를 놓쳤다.
라운드 4: 응답 결속과 구조화 필드 위생이다 — 시작에서 본문만 빼고 응답을 위조하면
통과했고, `settings.note`·`items` 의 임의 문자열이 루트 input·generation metadata 로 실렸다.
"구조화 필드라서 안전하다" 는 근거가 아니었다. 이제 `_clean()` 이 이름별 규칙을 걸고
모르는 이름은 안 내보낸다.
가드를 하나씩 되돌려 각각 빨개지는 것을 확인했다. mutation 이 두 번 초록이었다 —
그때마다 검사가 약한 자리였고(system 검사가 user 위조를 가렸고, 함수만 보고 배선을
안 봤다) 검사를 조였다. 전체 377 통과.
라운드 5 에서 접근을 바꿨다. 다섯 라운드 P0 열둘이 전부 같은 결함이었다 —
지적된 출구마다 게이트를 달아 열거에서 빠진 면이 다음 라운드에 나왔다. 이제
`plan()` 이 돌려주는 모든 Op 가 `guard()` 하나를 지나고, 기준은 실측(실행기 클래스·
실제 H4 로그)이고, 검사는 실제 회차 로그 1,020 이벤트로 크래시·보존·차단 셋을
같이 본다. 규칙은 허브 위키 `craft/gate-the-exit-not-the-callers` 가 소유한다.
라운드 6: 통로를 만들어도 전송 루프가 그 통로를 안 지나고 span 에 직접 적고 있었고,
`"값만 보고 구분할 수 없다"` 를 평문 허용의 근거로 쓴 것이 P0 였다 — 기본을 요약으로
뒤집었다. 라운드 7: P1 없이 P0 둘이고 둘이 같은 원리였다 — 기본을 뒤집고도 평문
예외를 패턴으로 둔 자리가 남아 있었다(map 키, `phase`·`error_type`).
모양이 값을 증명한다고 볼 수 있는 것은 hash 뿐이다 — 나머지는 생산자 집합과 대조한다.
전체 387 통과.
라운드 8: 남은 모양 예외 네 자리(자산 키·공고 id·버전·span 이름)를 전부 집합 대조로
바꿨다. 이제 모양으로 평문을 통과시키는 자리가 0개다 — 남은 정규식은 hash 와, 조각이
이미 위생을 지난 이름뿐이다. 오염 스윕도 토큰형까지 넓혔다.
라운드 9: P0 하나·P1 하나로 절반이 됐다. hash 가 마지막 모양 예외였다 —
`code_sha256="deadbeef"*8` 이 통과했다. 재계산할 수 있는 digest 열한 개와 대조하고
나머지는 요약한다(요약해도 동일성 비교는 된다). 생산자가 직접 쓰는 오류형
`ResponseCountMismatch` 도 집합에 넣었다.
라운드 10: P0 없이 P1 하나·P2 하나. P1 은 라운드 9 수리가 만든 회귀다 — 요약된
hash 의 앞 일곱 자가 전부 `sha256:` 이라 서로 다른 회차가 같은 trace 이름이 됐다.
`_clean()` 반환값만 비교한 검사가 소비 경로를 놓쳤다.
라운드 11 에서 새 발견 없이 `머지 허용`을 받았다 — 열한 라운드에 P0 스물셋·P1 열하나·
P2 넷이었고 P0 는 라운드 8 까지 나왔다.
실행 핀은 코드 `e6d9b98` · 노트북 `9940b50`(v13 수리 + 관측·감사 + 라운드 1~10 수리).
머지된 PR #82(`wiki-rag-pilot`)는 다른 가설이고 두 회차에서 기각됐다 — 1,200회 호출에서
검증된 참여 조항 인용 0건이다. 법령 원문을 넣는 것만으로 v20 이 안 움직인 실측이므로
A8 기대값을 낮춘다. 그쪽은 —
...
934자는 직접 조회 확인, 약 623토큰·baseline 초과 0/200은 추정이다.
...
그 파일럿의 `raw` 블록(12 span·3,843자)의 부분집합이고, 그 블록으로도 v20 TP는 control과
...
검증된 참여 조항 인용이 1,200회 호출에서 0건이고, 유일한 인용 1건(`PPS-DEV-068`)은 원문에 없는 문장이었다.
...
정탐과 오탐에서 같은 값이다. v24는 [세 축 진단](../reports/team-c/v24-not-gateable/README.md)에 더해
빈 근거 22건이 게이트를 통과하는 구멍과 "다르면 위반"이 아닌 반례 3건을 새로 남겼다.
...
부재탐지 5개는 전부 작동하는 항목의 음성 쌍둥이이고, 공통 병목은 공고문에서
...
건강하나 v23 규칙은 0.05배라 비공개 집합 재현율 경고가 붙는다.
...
`unlabeled40`: done / 표본 설계 실패, 라벨 40건은 확보. 2026-09-22.
astra 후보 넷(v3·v4·v13·v17)을 dev 밖에서 검증하려 무라벨 40건을 Opus 5 로 라벨링했으나
층을 항목의 텍스트 전제로 짜서 v4·v17·v13 양성이 0건이었다 — 항목 양성률이 3% 라
그 방식으로는 안 걸린다. 전 항목 음성 20건은 오탐 측정의 분모로 남는다.
전문과 다음에 제대로 하는 법은 [결과](../reports/label-compare/unlabeled40-result.md)가 소유한다.

`prompt-from-labels`: **전제 만료.** 대상 항목 v8·v14·v15 의 TP=0 이 이미 해소됐다 —
2026-09-22 재생에서 v8 6/0/0(F1 1.000) · v14 7/2/1(0.824) · v15 4/1/2(0.727) 이고
티켓 기준선은 Macro 0.2208 인데 현재는 0.6103 이다. 그 사이 다른 작업이 셋을 다 고쳤고
티켓은 `ready` 인 채로 남아 있었다. 가설을 다시 세우려면 대상·기준선이 모두 달라지므로
이 문서에 얹지 말고 새 티켓을 쓴다. 아래는 당시 설계다.

원래 계획: 그 33건의 근거 인용으로 Gemma 프롬프트의 v8·v14·v15 질문을 고치고
...
- 근거를 dev 33건에서 뽑았으므로 채점을 본 33건 / 못 본 167건으로 나눠 따로 낸다.
...
- 골든 대조: 원본 `colab-results-1789621345861123113.zip`(SHA-256 `335796bc…feec8`)과 `submit.zip`
  ...
  바이트 불일치 0개다. `manifest.json`에서 다른 칸과 이유는 아래가 전부다.
...
- 본 적 없는 ZIP 6건: `Downloads/123/`의 과거 회차를 대역 `submit.zip`과 짝지어 임시 루트에 돌렸다.
  5건이 손 수정 0으로 등록됐고 점수·커밋이 기존 색인 기록과 모두 일치했다
...
- 노트북 품질 게이트가 이번엔 반대로 뒤집혔다. `validation.json`이 `quality_pass: false`이고,
  ...
  run4가 같은 기준선을 +0.00000477로 넘겨 통과한 바로 그
...
- 왜 흔들림을 임계값으로 안 쓰나: 한 쌍의 관측이다. 도구는 실측 대비 배수만 찍고
