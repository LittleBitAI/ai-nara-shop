...
dev Macro F1 0.2208013652894021을 확인했다. 현재 후보는 응답 분할 복구와
v10·v11·v13 법령/품목 조회 파일럿을 추가했으며 **후보의 실제 GPU 성공·점수 개선은 미확인이다.**
...
7. 통과하면 `validation.json`을 만들고 **실제로 검증한 submit.zip**을 그대로 다운로드한다. 재패키징하지 않는다.
   `quality-comparison.json`은 같은 dev 입력·정답 해시를 확인한 뒤 이전 F1과의 차이 및 대상 3항목 지표를 남긴다.
   `colab_pass`는 실행 검증이며 점수 개선을 뜻하지 않는다. 개선 여부를 확인하기 전 후보를 채택하지 않는다.
