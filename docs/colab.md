...
기본 실행은 Colab에서 git clone으로 준비한다. 노트북만 열면 코드·공개 입력을 가져와
...
clone/업로드로 준비한 뒤에는 노트북에 추론 코드를 복제하거나 다른 코드로 바꾸지 않고 제출 ZIP의 script.py를 실행한다.

## 실행 순서

1. [Colab](https://colab.research.google.com/)에서 파일 → 노트북 업로드로 `colab-baseline.ipynb`를 연다.
...
4. uv로 Python 3.12.13을 준비하고 별도 venv에 서버 명세 버전을 설치한다. uv는 Colab 준비 도구이며 제출물에 포함하지 않는다.
...
같은 코드의 Colab dev 건당 3.469초를 1,853건에 곱한 6,428초와 어긋난다.
**최신 실측은 `57761ff`의 4,281초(59.5%)이고 남은 여유는 2,919초다** — 추가 호출을 200건에서
105건으로 줄인 최적화가 서버에서 재현됐다. 모델 호출을 늘리는 변경은 이 여유 안에 들어와야 하며,
전건 추가 호출 한 단계가 약 1,618초다. Colab 회차 횟수 제한은 2026-09-20부터 없지만
서버 시간 한도는 그대로다.
[제출 장부](../reports/submissions.json)와 [실행 기록](runs.md)을 본다.
...
2026-09-20부터 원응답 보존은 기본 실행이며, 별도 항목 질의만 선택한다.
노트북 9절은 `RUN_DIAGNOSTIC=True`, `DIAGNOSE_ITEMS=""`가 기본이다.
진단 회차 자체는 서버 진입점 검증 통과가 아니며 제출 후보를 만들지 않는다. 결과는 `results/`에 들어가
마지막 로그 셀의 ZIP에 함께 담긴다. `check_live`는 이 회차에 적용하지 않는다.

[W5](workflow.md#w5-실험검증)에 따라 채택 후보는 같은 ZIP으로 한 번 더 실행해 현재 코드의
churn을 잰다. 두 원응답 회차의 ZIP/코드·입력·모델·설정 동일성을 확인한 뒤 비교한다.
과거 29~45셀 / 0.000008~0.004689는 새 프롬프트의 허용선이 아니다.
노트북 재실행은 새 출력 작업 폴더에서 같은 업로드 번들로 시작하고 두 결과 ZIP을 보관한다.
