...
| 2026-09-18 | `57761ff` | **채점 대기** | — | — |

`57761ff`는 B의 근거 대조 4항목과 D의 참가자격 4항목이 들어간 첫 제출입니다.
같은 ZIP의 dev 200건은 **0.3719775475710831**입니다([colab-1789716500947261692](../reports/runs/colab-1789716500947261692/)).
**점수가 오기 전까지 채점된 제출로 세지 않습니다.** 예측값과 그 근거는
[제출 장부](../reports/submissions.json)의 `predicted_leaderboard`가 소유합니다.
...
채점된 회차는 **6,192초로 한도의 86.0%**를 썼습니다. 남은 여유는 **1,008초(16분 48초)**입니다.
건당 3.3416초였고 한도는 건당 3.8856초입니다.
...
| 실측 | 6,192초 (86.0%) |
| 여유 | 1,008초 |
| 건당 실측 / 한도 | 3.3416초 / 3.8856초 |

**모델 호출을 늘리는 변경은 이 여유 안에 들어와야 합니다.** 항목별 추가 호출·재시도 확대·
문서 예산 증가는 전부 여기에 부딪힙니다. 총시간에는 모델 로드와 입출력이 포함되며 서버의
