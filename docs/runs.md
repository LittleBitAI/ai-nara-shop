...
| `colab-1790318892216968298` | `ca352d6` | 0.7741694920003743 | NVIDIA A100-SXM4-40GB, vLLM 0.26.0, CUDA 13.0 | 포함 | [reports/runs/colab-1790318892216968298/](../reports/runs/colab-1790318892216968298/) |
| `colab-1790396310565903096` | `ea1d8ad` | 0.756719945813083 | NVIDIA A100-SXM4-40GB, vLLM 0.26.0, CUDA 13.0 | 포함 | [reports/runs/colab-1790396310565903096/](../reports/runs/colab-1790396310565903096/) |
| `colab-1790432295199698396` | `772ca12` | 0.822748901425372 | NVIDIA A100-SXM4-40GB, vLLM 0.26.0, CUDA 13.0 | 포함 | [reports/runs/colab-1790432295199698396/](../reports/runs/colab-1790432295199698396/) |
| `colab-1790445336782946136` | `772ca12` | 0.820764774441~0.828051931728 (6회) | NVIDIA A100-SXM4-40GB, vLLM 0.26.0, CUDA 13.0 | 포함 | [reports/runs/colab-1790445336782946136/](../reports/runs/colab-1790445336782946136/) |
...
1,200회 호출에서 검증된 참여 조항 인용이 0건이라는 것이 기각 근거입니다.
[셀 단위 감사·24항목 지표·채택 판단](../reports/wiki-rag-pilot/results.md).

현재 전체 파이프라인 dev GPU 최고는 0.599315859078, 서버 최고는 0.5084137874
(모두 `18f07e5`)로 이번 파일럿에 의해 갱신되지 않았습니다. CPU 합본 0.620382330538과도 구분합니다.

**서버 최신은 2026-09-21 `3a30167`의 0.5057795952이고 최고가 아닙니다.** A4 적용범위 게이트만
채택한 제출이며 dev 재생 +0.017878이 서버에서 −0.0026341922로 부호를 바꿨습니다(전이율 −0.147).
v23 규칙의 무라벨 배율 0.05 경고가 실측으로 확인됐습니다. 게이트는 미채택이고 `main`의
운영 `script.py`에는 들어가 있지 않습니다. [결과 분석](../reports/submission-a4-server-result.md).
...
| 2026-09-20 | `18f07e5` | 채점됨 | 0.5084137874 | 6,380초 (106분 20초) |
| 2026-09-21 | `3a30167` | 채점됨 | 0.5057795952 | 6,300초 (105분) |

`57761ff`는 B의 근거 대조 4항목과 D의 참가자격 4항목이 들어간 첫 제출이고
`654c556` 대비 +0.0781587418입니다.

### 9/18 당시 전이율 해석 — 이후 관측으로 고정 규칙 철회

아래는 첫 두 채점의 역사 기록입니다. 9/20의 전이율은 0.926으로 달라져
아래 “dev 이득에 0.5를 곱한다”는 운영 규칙은 철회됐습니다. 최신 해석은 `submissions.json`이 소유합니다.
...
규칙 없는 같은 프롬프트 회차가 dev 0.218211이므로 규칙의 dev 이득은 0.153767이고
같은 규칙의 서버 이득은 0.078159입니다 — 전이율 약 50.8%.
...
0.2197036943입니다. 차이는 0.0010840169901622787입니다.
...
총시간 -30.9%. dev에서 -23.8%였고 서버에서 더 크게 줄었습니다. 여유가 2.90배입니다.
