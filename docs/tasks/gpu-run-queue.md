...
**딱 두 군데**를 고치고 위에서부터 전부 실행한다.

**① 맨 위 셀**

```python
REPO_REF = "main"                          # 이것을
REPO_REF = "exp/round1-absence-evidence"   # 이렇게
```

**② 아래쪽 `## 9. 항목 진단 전용 회차 (선택)` 셀**

```python
RUN_DIAGNOSTIC = False        # 이것을      RUN_DIAGNOSTIC = True
DIAGNOSE_ITEMS = ""           # 이것을      DIAGNOSE_ITEMS = "v14,v15,v17"
```

`DIAGNOSE_IDS` 는 빈 문자열 그대로 둔다(dev 200건 전체라는 뜻이다).
