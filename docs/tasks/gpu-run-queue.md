...
churn을 재고 그 위에서 효과를 읽는 것이 **예외가 아니라 기본**이 된다.
...
1. **같은 코드 두 번.** 그 차이가 그날 그 코드의 churn이고, 효과는 그 위에서만 읽힌다.
   지금 churn 기준(4,800셀 중 29~45개, Macro F1 0.000008~0.003129)은 과거 열 쌍의 값이다.
...
0.000008~0.003129)은 **과거 열 쌍의 값**이며([근거](../../reports/runs/reproducibility.md))
