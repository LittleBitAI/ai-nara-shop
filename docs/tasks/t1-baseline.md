...
## 2026-09-17 추가 추론 비용 축소

- 입력: 654c556 및 colab-results-1789621345861123113.zip. dev 기본 363초,
  추가 330.8초. v10/v11 변화 0건, v13 양성 107건 중 101건 해제, 최종 F1 0.2207877113.
- 출력: 기본 v13 양성 공고만 v13 재검증. v10/v11와 음성 공고의 중복 추론 제거,
  v13용 법령·지시 및 중복 근거 출력 축소. 기본 24항목·16,000자·복구는 유지한다.
- 수정 범위: script.py, tests/test_baseline.py, tests/test_package.py,
  notebooks/colab-baseline.ipynb, docs/colab.md, 이 작업서·.wiki/plan-active.md,
  reports/t1-baseline/selective-v13.md, artifacts/selective-v13/.
- 통과 조건: 선택 공고만 호출, 비연속 ID 대응·23항목 보존·0건 호출 생략·실패 보존,
  토큰 예산·과거 출력 재조합·패키징·노트북 건수 검사. 새 실제 시간/점수는 Colab에서 확인한다.
- 규칙: A1·A4·A5·A9·A10, R1·R3·R4·R7~R9·R11·R15·R17~R22.
  다른 공고·dev 정답으로 선택하지 않는다. 기존 기본 음성의 미탐은 재검증으로 복구하지 못한다.
  서버 채점 중인 ZIP은 변경하지 않는다. 상태: 구현·로컬 검증 완료, 실제 GPU/서버 검증 대기.
- 결과: 27개 로컬 검사 및 비연속 실패 위치 추가 검사·패키징 통과. 최신 기록 기준 추가 호출
  200→107건, 로컬 입력 토큰 2,030,736→965,553. 저장 응답 재조합은 기존 최종 CSV와 같다.
  새 프롬프트의 실제 속도·점수는 미측정. [실행 증거](../../reports/t1-baseline/selective-v13.md) 참조.

## 2026-09-17 성공 제출본 대조에 따른 응답 정규화

- 입력: 팀원 성공 제출 폴더의 script.py(제공 baseline과 동일), 최초 실패 ZIP,
  현재 8327eac와 이전 Colab 로그. 최초 서버 응답 자체는 알 수 없다.
- 출력: 의미가 같은 이진값을 정수 0/1로 정규화하고 추가 설명 필드를 무시한다.
  누락·모호한 판정은 분할 복구한다. 빈 응답을 0으로 채우는 팀원 경로는 도입하지 않는다.
- 수정 범위: script.py, tests/test_baseline.py, 이 작업서, .wiki/plan-active.md,
  reports/t1-baseline/response-normalization.md, artifacts/response-normalization/.
- 통과 조건: 변경 전 청크 62의 불필요한 재시도 재현, 변경 후 정규화·추가 필드 허용,
  결손/모호한 값/비정상 사실 거부, 기존 복구·기본 판정 보존·패키징 검사.
  정상 입력 프롬프트·문서 예산·점수 기준 유지. 실제 GPU/서버 통과는 별도 검증이다.
- 규칙: 개발·로컬 검증 A1·A4·A5·A9, R1·R3·R4·R7~R9·R15·R17~R22.
  서버 로그 회수를 요구하지 않는다. 상태: in_progress.
- 결과: 수정 전 16조합 실패 재현 후 로컬 검사 26개·패키징 통과.
  dev 200건 프롬프트와 정상 판정 결과를 보존했다. 새 GPU/서버 성공은 미검증이다.
  [비교·실행 증거](../../reports/t1-baseline/response-normalization.md).

## 2026-09-17 실행 중단 방지 우선 보완

- 목표: 최초 서버 재시도 실패 대응을 성능보다 우선한다. 원래 서버 원인은 미확정이다.
- 입력: 최초 제출 ZIP, e9e4022 코드, 최신/이전 Colab 진단. 최신 210건 정상,
  이전 별도 단계 출력 잘림 3건 분할 복구. 최신 품질 기준은 미달이다.
- 출력: 실패한 기본 6항목 그룹만 단일 항목으로 복구, 추가 단계 실패 시 해당 공고의
  검증된 기본 24항목 응답 보존. 정상 기본 응답 없는 공고는 성공 처리하지 않는다.
- 수정 범위: script.py, tests/test_baseline.py, tests/test_package.py,
  notebooks/colab-baseline.ipynb, 이 작업서, .wiki/plan-active.md, reports/t1-baseline/,
  artifacts/response-recovery/. 프롬프트 언어·점수 임계값·판정 조건은 유지한다.
- 통과 조건: 실패 재현 후 복구/보존/실패 로그 회귀, 실제 기본 호출 없는 성공 금지,
  Colab의 기본 성공 및 추가 성공/보존 건수 대조, ZIP 검사. 실제 GPU/서버 통과는 별도다.
- 규칙: 개발·검증 A1·A4·A5·A9, R1·R3·R4·R7~R9·R15·R17~R22.
  외부 호출·정답/ID별 분기·임의 0 채움 없음. 상태: in_progress.
- 결과: 수정 전 중단 2경로 재현 후 로컬 25개 검사·패키징 통과.
  [실행 기록](../../reports/t1-baseline/response-recovery.md). 새 ZIP의 실제 GPU/서버 성공은 미검증.
  서버 로그는 사용자가 열 수 없으므로 Colab 검증에 사용하며 서버 로그 회수를 요구하지 않는다.

## 2026-09-17 영어 지시·근거 확인 후보

- 사용자 요청: 과거 실행을 바탕으로 실제 품질 기준을 통과하는 후보를 만든다.
  모델 지시는 영어, 법적 용어·항목명·제공 원문은 한국어로 유지한다.
- 입력: 57c78cf 및 이전 Colab 로그, 제공 공고·법령·품목·dev 정답.
  57c78cf는 기본 F1 0.211150, 최종 0.211008로 실패. 잘린 응답 3건은 분할 복구 성공.
- 출력: 영어 기본/별도/재시도 지시, 3항목의 품목 적용·자격 사실을 구조화해 검증한 판정,
  같은 실행 기본/후처리 비교 및 제출/Colab 후보. 실제 품질 기준은 완화하지 않는다.
- 수정 범위: script.py, tests/test_baseline.py, tests/test_package.py,
  notebooks/colab-baseline.ipynb, docs/colab.md, 이 작업서·docs/tasks.md·.wiki/plan-active.md,
  reports/t1-baseline/, artifacts/english-evidence/.
- 통과 조건: 사실 스키마·근거 원문·품목 코드 검증, 실패 재시도·21항목 보존,
  영어 지시·한국어 원문 보존, 토큰·mock·패키징·Colab 회귀 검사.
  실제 다운로드 조건은 최종 F1 > 같은 실행 기본 F1 및 최종 F1 ≥ 0.2208013652894021.
- 규칙: A1·A4·A5·A6·A9·A10, R1·R3·R4·R7~R9·R11·R15·R17~R22.
  정답/ID별 분기 없음. dev 개발 점수를 독립 평가로 취급하지 않는다.
  독립 리뷰 생략·커밋/PR/머지 권한 유지. 새 GPU 실행 전에는 품질 통과를 주장하지 않는다.
- 결과: 로컬 24개 검사·후보 패키징 통과. 영어 기본/별도 프롬프트 dev 200건에서 추가 문서
  축소 0건, 최대 12,135/14,066토큰. [실행 기록](../../reports/t1-baseline/english-evidence.md).
  실제 품질 기준 통과는 사용자 Colab 재실행 대기다.

## 2026-09-17 품목 적용 범위·자격 조건 보완

- 입력: `40e2cc6` 실제 Colab 로그 `colab-results-1789609860516528072.zip`,
  과거 `1c64604` 로그, 공개 dev 공고·정답·제공 법령/품목 CSV.
- 가설: 품목 문자열 일치를 적용 확정으로 오인하고, 서비스 정식 품명 조회 누락과
  중·소기업/소기업 범위 및 자격 요구/일반 안내 혼동이 3항목 오답에 기여한다.
- 출력: 조회 출처·미일치 코드·서비스 보조 목록과 명확한 적용/자격 판정 지시를 갖춘 후보.
  기본 24항목 입력, 분할 재시도, 다른 21항목 보존, Colab 비교·다운로드 기준은 유지한다.
- 수정 범위: `script.py`, `tests/test_baseline.py`, 이 작업서·`docs/tasks.md`·
  `.wiki/plan-active.md`, `reports/t1-baseline/`, `artifacts/sme-applicability/`.
- 통과 조건: 조회 우선순위·출처·특이사항·공고 독립성·입력 보존 회귀 검사,
  기본 dev 메시지 동일성·고정 토크나이저 예산·패키징 검사. 실제 F1·시간은 Colab 재검증한다.
- 규칙: 개발/로컬 검증, A1·A5·A6·A9·A10, R1·R3·R4·R7~R9·R11·R15·R17~R21.
  dev 오답을 보고 개발한 후보이므로 dev는 독립 검증셋이 아니다. ID별 규칙·정답 입력 없음.
  기존 리뷰 생략·커밋/PR/머지 권한을 유지한다.
- 결과: 구현·로컬 22개 검사 통과. 기본 메시지 200건 동일, 별도 단계 최대 14,261토큰,
  추가 문서 축소 0건. ZIP/Colab 번들 생성·압축 해제 mock 검사 통과.
  [분석·검증 기록](../../reports/t1-baseline/sme-applicability.md). 새 후보의 실제 GPU 검증 대기.
