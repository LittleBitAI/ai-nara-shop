...
| 평문 | 왜 |
| --- | --- |
| 생산자에서 읽은 enum 값 | `mode`·`token_count_kind` 는 실행기 클래스, `phase` 는 소스, `arm` 은 `DEV_ONLY`, `error_type` 은 `builtins` 의 예외 클래스 |
| `MODEL_ID`·`MODEL_REVISION`·`QUANT` 와 같은 문자열 | 상수와 대조했다 |
| hash·항목명 | 모양이 값을 증명한다 |
| 순수 숫자 버전(`0.26.0`·`2.11.0+cu130`) | 각 묶음 세 자리, 접미는 영문자로 시작. 어느 vLLM 이었는지는 회차 판정의 근거다 |
| 공고 id | **회차가 선언한 `dev_ids`** 와 대조한다. 고정 dev 로 하면 제출 입력으로 돈 회차의 id 가 전부 요약돼 관측이 못 쓰게 된다 |
| map 의 키 | 자료 이름은 드라이브 문자·상대 상승을 거부하는 모양, 패키지 이름은 `script.py` 가 세는 목록 |
