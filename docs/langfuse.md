...
| 평문 | 왜 |
| --- | --- |
| 생산자에서 읽은 enum 값 | 집합이 코드에 있다(`mode`·`token_count_kind` 는 실행기 클래스) |
| `MODEL_ID`·`MODEL_REVISION`·`QUANT` 와 같은 문자열 | 상수와 대조했다 |
| hash·항목명 | 모양이 값을 증명한다 |
| 순수 숫자 버전(`0.26.0`·`2.11.0+cu130`) | 엔트로피가 없다. 어느 vLLM 이었는지는 회차 판정의 근거다 |
| key·이름을 만드는 짧은 식별자(`id`·`arm`·`sample`·`phase`) | 요약하면 trace 를 못 읽는다. 그 자리는 1층이 dev id·군으로 대조한다 |

**요약은 유실이 아니다** — 동일성과 변화는 남으므로 "같은 설정으로 돌았나" 는 답할 수 있다.
모르는 이름은 요약도 안 하고 아예 버린다.

`error_type` 은 예외 **클래스 이름**이라 CamelCase 만 평문이다.
